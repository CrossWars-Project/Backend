...
def generate_crossword_with_theme(theme):
    ...
    try:
        ...
        if response.status_code == 200:
            print(f"[{datetime.now()}] Successfully generated crossword with theme '{theme}'")
            return True
        else:
            print(f"[{datetime.now()}] Failed to generate crossword: {response.status_code}")
            return False
    except Exception as e:
...
def scheduler_loop():
    ...
    if not SOLO_JSON_PATH.exists() or not BATTLE_JSON_PATH.exists():
        print(f"[{datetime.now()}] One or more crossword files missing. Generating now...")
        time.sleep(2)  # Wait for server to be fully ready
        generate_daily_crosswords()
    else:
        print(f"[{datetime.now()}] Both crossword files exist. Waiting for next scheduled time.")
    ...
    while True:
        try:
            ...
            print(f"[{datetime.now()}] Next deletion in {hours_remaining:.1f} hours (at 11:59 PM)")
